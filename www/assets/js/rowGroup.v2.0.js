...
RowsGroup.prototype = {
	...
	_mergeColumn: function(iStartRow, iFinishRow, columnsIndexes)
	{
		...
		while(columnsIndexesCopy.length != 0) {

			var currentColumn = columnsIndexesCopy.shift();
			currentColumn = this.table.column(currentColumn, ShowedDataSelectorModifier);
			columnNodes.push(currentColumn.nodes());
			columnsValues.push(currentColumn.data());
		}

		var nextLevelSequence = new Array();
		var currentLevelSequence = new Array();
		var sequence = { "start": 0 , "inner": 0, "finish": 1 };


		while(sequence.inner <= iFinishRow) {

			if (columnsValues[0][sequence.start] === columnsValues[0][sequence.finish]) {
				$(columnNodes[0][sequence.finish]).hide();
				sequence.inner++;
				sequence.finish++;
			} else {
				$(columnNodes[0][sequence.start]).show();
				$(columnNodes[0][sequence.start]).attr('rowspan', sequence.finish - sequence.start);
				currentLevelSequence.push({ "start": sequence.start, "finish": sequence.inner });
				sequence.start = sequence.finish;
				sequence.inner = sequence.finish;
				sequence.finish++;
			}
		}

		for(var jColumn = 1; jColumn < columnNodes.length;) {


			jQuery.extend(sequence, currentLevelSequence.shift());
			sequence.inner = sequence.start;

			do {

				sequence.inner++;

				if (columnsValues[jColumn][sequence.start] === columnsValues[jColumn][sequence.inner]) {
					$(columnNodes[jColumn][sequence.inner]).hide();

				} else {
					$(columnNodes[jColumn][sequence.start]).show();
					$(columnNodes[jColumn][sequence.start]).attr('rowspan', sequence.inner - sequence.start);
					nextLevelSequence.push({ "start": sequence.start, "finish": sequence.inner - 1 });
					sequence.start = sequence.inner;
				}

			} while(sequence.inner <= sequence.finish) 
			if(sequence.inner > iFinishRow) {
				jColumn++;

				currentLevelSequence = nextLevelSequence.slice();
			}
		}
